...
from gsasrec_torchrec import TorchRecGSASRec
...
for epoch in range(config.max_epochs):
    ...
    if evaluation_result[config.val_metric] > best_metric:
        ...
        model_name = (
            f"models/torchrec-gsasrec-{config.dataset_name}-step:{step}-t:{config.gbce_t}"
            f"-negs:{config.negs_per_pos}-emb:{config.embedding_dim}"
